...
MODEL_PATH = BASE_DIR / "ai-model" / "tomato_disease_model.h5"
...
def get_model():
    ...
    try:
        ...
        return load_legacy_mobilenetv2_model()


def load_legacy_mobilenetv2_model():
    model = Sequential(
        [
            ...
            Dense(len(TOMATO_CLASSES), activation="softmax", name="dense_1"),
        ],
        name="sequential",
    )

    load_h5_weights_by_name(model, MODEL_PATH)
    return model
...
def analyze_crop_image(image_file):
    ...
    disease = get_class_name(class_index)
...
def get_class_name(class_index):
    if class_index >= len(TOMATO_CLASSES):
        return f"Tomato class {class_index}"

    return TOMATO_CLASSES[class_index]
